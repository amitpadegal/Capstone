...
def evaluation(model, data_loader, device, config) :
    ...
    for n, (image, question) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):        
        # print(n, image.shape if hasattr(image, "shape") else len(image))
        image = image.to(device,non_blocking=True)             

        if config['inference']=='generate':
            print(n)
            answers, res = model(image, question, "blip_scienceqa.json",train=False, inference='generate') 
        ...
        if n == 1500:
            ...
            df.to_csv("scienceqa_results.csv", index=False)
            with open("blip_scienceqa.jsonl", 'w') as f:
...
def main(args, config):
    ...
    datasets = create_dataset('scienceqa', config) 
    print(len(datasets))
