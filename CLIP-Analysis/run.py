...
def evaluation(model, preprocess, data_loader, device):
    ...
    for n, (image, question, question_id) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):        
        ...
        with torch.no_grad():
            ...
            output_features = image_features @ text_features.t()
            image_features = image_features / image_features.norm(dim=1, keepdim=True)
            text_features = text_features / text_features.norm(dim=1, keepdim=True)
            output_features = image_features * text_features
            image_tmp, question_tmp, question_states_tmp = image_features.squeeze(0), text_features.squeeze(0), output_features.squeeze(0)
